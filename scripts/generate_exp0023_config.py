#!/usr/bin/env python3
"""Generate or verify the frozen EXP-0023 exact 500K training config."""
...
def render(source: dict, run_root: Path) -> str:
    ...
    if config["task"] != "minecraft_diamond" or config["seed"] != 0:
        raise ValueError("Unexpected Minecraft source protocol")
    if config["run"]["steps"] != float(SOURCE_STEP):
        raise ValueError("EXP-0023 source must end at step 200000")
    if config["run"]["train_ratio"] != 32.0:
        raise ValueError("Unexpected Minecraft train ratio")
