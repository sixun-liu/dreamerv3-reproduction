...
readonly ROOT=/root/autodl-tmp/Runs/EXP-0018__minecraft-exact-stop__s000__100k-to-100040-env__20260812T090000Z
readonly CONFIG=${CONTROL}/docs/reproduction/configs/exp0018_minecraft_s000_100040_env.yaml
readonly MATRIX=${CONTROL}/docs/reproduction/configs/exp0018_exact_stop_matrix.yaml
readonly BASELINE=${CONTROL}/docs/reproduction/configs/exp0012_minecraft_s000_100k_env.yaml
readonly SOURCE_STEP=100000
readonly FINAL_STEP=100040
readonly ENVS=4
...
if [[ -e "${ROOT}" || -e "${STARTED}" ]]; then
  echo "Refusing duplicate EXP-0018 launch" >&2
  exit 20
fi
...
"${PYTHON}" "${CONTROL}/scripts/generate_exp0018_config.py" \
  --baseline "${BASELINE}" --run-root "${ROOT}" --output "${CONFIG}" \
  --check >/dev/null
(cd "${RUNTIME}" && \
  "${PYTHON}" -m pytest -q embodied/tests/test_train_exact_steps.py >/dev/null)
...
printf \
  '{"schema_version":1,"experiment_id":"%s","control_commit":"%s","runtime_commit":"%s","workflow_commit":"%s","config_sha256":"%s","matrix_sha256":"%s","source_step":%d,"absolute_final_step":%d,"environment_count":%d}\n' \
...
/usr/bin/time -v -o "${ROOT}/resource_time.txt" \
  env \
    ...
    --run.train_ratio 32 --run.log_every 1 --run.report_every 120 \
