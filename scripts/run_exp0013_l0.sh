...
readonly ROOT=${RUN_ROOT}/l0-envs2
readonly STARTED=${RUN_ROOT}.started
...
fail() {
  local status=$?
  printf \
    '{"experiment_id":"%s","stage":"l0-envs2","failed_at":"%s","exit_code":%d,"wall_seconds":%d}\n' \
...
if [[ -e "${RUN_ROOT}" || -e "${STARTED}" ]]; then
  echo "Refusing duplicate EXP-0013 launch" >&2
  exit 20
fi
...
printf '{"experiment_id":"%s","stage":"l0-envs2","started_at":"%s","pid":%d}\n' \
...
if not summary["passed"] or not cleanup["passed"]:
    raise SystemExit((summary.get("passed"), cleanup))
PY
if pgrep -f '[j]ava.*Malmo' >/dev/null || pgrep -f '[M]inecraft\.launchClient' >/dev/null; then
  echo "Minecraft process remained after L0" >&2
  exit 27
fi
printf \
  '{"experiment_id":"%s","stage":"l0-envs2","completed_at":"%s","exit_code":0,"wall_seconds":%d}\n' \
