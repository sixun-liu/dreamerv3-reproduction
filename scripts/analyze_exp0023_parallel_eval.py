#!/usr/bin/env python3
"""Verify EXP-0023 runtime-derived parallel Minecraft evaluation."""
...
def analyze(
    ...
    *,
    ...
    runtime_mappings = [row.get("milestone_runtime_indices", {}) for row in episodes]
    inventory_key_counts = [int(row.get("inventory_key_count", -1)) for row in episodes]
    runtime_mapping_valid = bool(
        len(runtime_mappings) == 3
        and all(set(mapping) == set(MILESTONES) for mapping in runtime_mappings)
        and all(count > 0 for count in inventory_key_counts)
        and all(
            len(set(mapping.values())) == len(MILESTONES)
            and all(0 <= int(index) < count for index in mapping.values())
            for mapping, count in zip(runtime_mappings, inventory_key_counts)
        )
    )
    cross_worker_mapping_consistent = bool(
        runtime_mapping_valid
        and len(set(inventory_key_counts)) == 1
        and all(mapping == runtime_mappings[0] for mapping in runtime_mappings[1:])
    )
    reset_milestones_zero = bool(
        len(episodes) == 3
        and all(
            set(row.get("reset_milestone_values", {})) == set(MILESTONES)
            and all(
                float(value) == 0.0
                for value in row["reset_milestone_values"].values()
            )
            for row in episodes
        )
    )
    protocol_ok = bool(
        evaluation.get("experiment_id") == expected_experiment_id
        ...
        and evaluation.get("milestone_mapping_source")
        == "runtime _MinecraftBase._inv_keys per worker"
    )
    ...
    integrity_passed = bool(
        ...
        and checkpoint_ok
        and runtime_mapping_valid
        and cross_worker_mapping_consistent
        and reset_milestones_zero
        and protocol_ok
        and returns_finite
    ...
    return {
        ...
        "integrity": {
            ...
            "runtime_mapping_valid": runtime_mapping_valid,
            "cross_worker_mapping_consistent": cross_worker_mapping_consistent,
            "reset_milestones_zero": reset_milestones_zero,
        ...
        "resource": resources,
        "video": video,
        "parallel_evaluator_integrity_passed": integrity_passed,
        "reference_speed_gate_passed": promoted,
        "verdict": (
