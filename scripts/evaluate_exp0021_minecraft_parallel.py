...
def make_single_episode_env(runtime: str, config, worker: int):
    ...
    return SingleEpisodeHold(dv3_main.make_env(config, worker))
...
def main() -> None:
    ...
    parser.add_argument("--experiment-id", default="EXP-0021")
    ...
    if args.episodes != 3:
        raise ValueError("EXP-0021 is frozen to exactly three workers and episodes")
    ...
    inventory_keys = [
        f"inventory/{item}" for item in mc.ALL_ITEMS if item != "log2"
    ]
    if obs_space["inventory_max"].shape != (len(inventory_keys),):
        raise ValueError("Static MineRL inventory ordering does not match runtime space")
    milestone_columns = {
