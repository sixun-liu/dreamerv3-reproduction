...
readonly MATRIX=${CONTROL}/docs/reproduction/configs/exp0021_minecraft_parallel_eval_matrix.yaml
readonly ROOT=/root/autodl-tmp/Runs/EXP-0021__minecraft-parallel-eval__s10000__3eps__20260812T120000Z
...
if [[ -e "${ROOT}" || -e "${STARTED}" ]]; then
  echo "Refusing duplicate EXP-0021 launch" >&2
  exit 20
fi
...
run_arm() {
  ...
  /usr/bin/time -v -o "${arm}/resource_time.txt" \
    env \
      ...
      "${PYTHON}" "${CONTROL}/scripts/evaluate_exp0021_minecraft_parallel.py" \
  ...
  "${PYTHON}" "${CONTROL}/scripts/analyze_exp0021_parallel_eval.py" \
    --run-dir "${arm}" \
