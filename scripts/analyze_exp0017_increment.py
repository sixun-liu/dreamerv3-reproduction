#!/usr/bin/env python3
"""Analyze EXP-0017 added replay, evaluation, resources, and 100K comparison."""
...
def learning_verdict(added_replay: dict, evaluation: dict) -> dict:
    l1_eval_success = evaluation["milestones"]["crafting_table"][
        "successful_episodes"
    ]
    advance_replay = added_replay["milestones"]["wooden_pickaxe"]["observed"]
    advance_eval = (
        evaluation["milestones"]["wooden_pickaxe"]["successful_episodes"] > 0
    )
    return {
        "l1_minimum_successful_episodes": 2,
        "l1_evaluation_successful_episodes": l1_eval_success,
        "l1_maintained": l1_eval_success >= 2,
        "advance_item": "wooden_pickaxe",
        "advance_observed_in_added_replay": advance_replay,
        "advance_observed_in_evaluation": advance_eval,
        "advance_observed": advance_replay or advance_eval,
        "hypothesis_supported": l1_eval_success >= 2 and (advance_replay or advance_eval),
    }
...
def build_figure(
    ...
    eval_x = np.arange(3)
    axes[0, 1].plot(eval_x, baseline_returns, "o-", label="100K", color="#8D99AE")
    axes[0, 1].plot(eval_x, current_returns, "o-", label="200K", color="#2A6F97")
    ...
    axes[1, 0].set_title("Added 100K replay milestone evidence")
    ...
    axes[1, 1].set_title("200K increment resource facts")
...
def main() -> None:
    ...
    if added_replay["source_unique_stepids"] != 100_000:
        raise ValueError("Unexpected source replay size")
    if added_replay["target_unique_stepids"] != 200_000:
        raise ValueError("Unexpected target replay size")
    if added_replay["new_unique_stepids"] != 100_000:
    ...
    summary = {
        ...
        "evaluation_100k": baseline_evaluation,
        "evaluation_200k": evaluation,
        ...
        "interpretation_boundary": (
            ...
            "This 200K run is still far below the paper-scale Diamond budget."
        ),
...
- 工程完整性：`passed`
- 200K 终点评测 L1（crafting_table）：`{verdict['l1_evaluation_successful_episodes']}/3`
- 新增 replay 出现 wooden_pickaxe：`{str(verdict['advance_observed_in_added_replay']).lower()}`
- 终点评测出现 wooden_pickaxe：`{str(verdict['advance_observed_in_evaluation']).lower()}`
- 预注册学习假说得到支持：`{str(verdict['hypothesis_supported']).lower()}`

## 资源

- 新增 100K 训练墙钟：`{resources['training_wall_seconds'] / 60:.2f}` 分钟
...
- 100K 与 200K 评测的 Minecraft 世界 seed 均不可控，因此只作描述性比较。
- 多环境新增 replay 是轨迹森林，首次位置不是严格全局环境时间。
- 200K 仍远低于论文 Diamond 量级，不构成论文钻石结果复现。
"""
