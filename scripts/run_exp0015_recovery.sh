...
readonly ROOT=/root/autodl-tmp/Runs/EXP-0015__minecraft-recovery__s000__100k-to-105040-env__20260812T071500Z
readonly CONFIG=${CONTROL}/docs/reproduction/configs/exp0015_minecraft_recovery_s000_105040_env.yaml
readonly MATRIX=${CONTROL}/docs/reproduction/configs/exp0015_minecraft_recovery_matrix.yaml
...
if [[ -e "${ROOT}" || -e "${STARTED}" ]]; then
  echo "Refusing duplicate EXP-0015 launch" >&2
  exit 20
fi
...
"${PYTHON}" "${CONTROL}/scripts/generate_exp0015_config.py" \
...
printf \
  '{"schema_version":1,"experiment_id":"%s","control_commit":"%s","runtime_commit":"%s","workflow_commit":"%s","config":"%s","config_sha256":"%s","matrix_sha256":"%s","seed":0,"source_step":%d,"absolute_final_step":%d,"step_unit":"environment steps"}\n' \
...
"${PYTHON}" "${CONTROL}/scripts/prepare_exp0015_recovery.py" \
  ...
  --output "${CLONE_MANIFEST}" \
  > "${ROOT}/recovery_clone_stdout.log" 2>&1 || fail $? clone
...
/usr/bin/time -v -o "${ROOT}/resource_time.txt" \
  env \
    ...
    --run.envs 2 --run.debug False --run.steps "${FINAL_STEP}" \
...
printf \
  '{"experiment_id":"%s","completed_at":"%s","exit_code":0,"source_step":%d,"absolute_final_step":%d,"added_environment_steps":%d,"training_wall_seconds":%d,"total_wall_seconds":%d,"output_bytes":%d}\n' \
  "${EXPERIMENT}" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "${SOURCE_STEP}" \
  "${FINAL_STEP}" "$(( FINAL_STEP - SOURCE_STEP ))" "${TRAIN_WALL_SECONDS}" \
  "${WALL_SECONDS}" "$(du -sb "${ROOT}" | awk '{print $1}')" > "${COMPLETED}"
